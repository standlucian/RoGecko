...
public:
     ...
     */
    ThreadBuffer(uint64_t size, uint64_t chunkSize, int moduleId, T defaultValue = NULL);
    ~ThreadBuffer();
...
template<class T>
ThreadBuffer<T>::ThreadBuffer(uint64_t _size, uint64_t _chunkSize, int _moduleId, T defaultValue)
